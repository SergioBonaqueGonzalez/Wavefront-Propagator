function [x2, Uout] = two_step_prop(Uin, wvl, d1, d2, Dz)
%{
function developed by:
- Jason D. Schmidt - Numerical Simulation of Optical Wave Propagation With Examples in MATLAB (2010)

function [x2 y2 Uout]  = two_step_prop(Uin, wvl, d1, d2, Dz)
%}
...
Uitm = 1 / (1i*wvl*Dz1) .* exp(1i*k/(2*Dz1) * (x1a.^2+y1a.^2)) .* ft2(Uin .* exp(1i * k/(2*Dz1) * (x1.^2 + y1.^2)), d1);
% observation plane
Dz2 = Dz - Dz1; % propagation distance
% coordinates
[x2, y2] = meshgrid((-N/2 : N/2-1) * d2);
% evaluate the Fresnel diffraction integral
Uout = 1 / (1i*wvl*Dz2) .* exp(1i*k/(2*Dz2) * (x2.^2+y2.^2)) .* ft2(Uitm .* exp(1i * k/(2*Dz2) .* (x1a.^2 + y1a.^2)), d1a);
Uout=rot90(Uout,2); %solve the rotation of the matrix
